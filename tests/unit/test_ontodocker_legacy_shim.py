...
class TestOntodockerLegacyShim(unittest.TestCase):
    ...
    def test_download_dataset_as_turtle_file_returns_path_and_warns_on_default(self):
        fake_client = mock.Mock()
        fake_client.datasets.download_turtle.return_value = Path("unused.ttl")

        with (
            TemporaryDirectory() as tmpdir,
            mock.patch("courier.ontodocker.OntodockerClient", return_value=fake_client),
            mock.patch("courier.ontodocker.Path.cwd", return_value=Path(tmpdir)),
    ...
    def test_send_query_requires_scheme_in_endpoint(self):
        with self.assertRaisesRegex(ValueError, "must include a URL scheme"):
            _ = courier.ontodocker.send_query(
                "example.org/api/v1/jena/ds/sparql",
                "SELECT ?a WHERE {}",
                columns=["a"],
            )
