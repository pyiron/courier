...
def download_dataset_as_turtle_file(
    ...
    *,
    ...
    _ = client.datasets.download_turtle(dataset_name.strip(), filename=turtlefile_name)
...
def send_query(
    ...
    *,
    ...
    base = f"{parts.scheme}://{parts.netloc}" if parts.scheme else parts.netloc

    # Reuse robust extraction logic from the new implementation.
    dataset = extract_dataset_names([endpoint.strip()])[0]
